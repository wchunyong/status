...
macOS 菜单栏轻量系统监控 —— 实时显示**网络上下行速率、内存占用、CPU 占用**。原生 Swift，低占用、长时稳定，适配 macOS 26 液态玻璃（14+ 渐进增强）。

> 进度：**Phase 0–4 完成**（采集 + 状态栏 + 菜单详情 + 设置界面），**Phase 5 待人工**（签名/长跑/可视化 QA）。详见 [`docs/ROADMAP.md`](docs/ROADMAP.md)。

## 功能
- 状态栏常驻：`↓5.20 ↑0.32 MB/s · 12.4 / 24.0 GB · 38%`
- 点击图标弹菜单：网络/内存/CPU 明细 + 设置/退出
- 设置五 Tab：通用、网络、内存、CPU、显示（单位/格式/顺序/显隐/紧凑/外观/自启动）
...
```
Sources/
├── StatusCore/          # 纯逻辑库（Swift 6，48 单测覆盖）
│   ├── Monitoring/      # CPU/Mem/Net 采集（Mach + sysctl）+ Sampler + SystemMonitor(actor)
