...
> D 编号的**权威详记**在 `docs/ROADMAP.md` §7 ADR（背景 / 决策 / 备选 / 后果）。变更**先改 ROADMAP §7**，再同步本节速查，保持单一事实来源。
...
| D7 | ~~风扇控制~~ | **v2.0 移除** — macOS 26+ 完全禁止用户空间 SMC 访问（kIOReturnNotPermitted 0xE00002C2），所有读/写操作被内核拒绝。详见 ROADMAP §7 ADR D7。
...
> ⚠️ **2026-07-23 移除**：macOS 26+ 完全禁止用户空间 SMC 访问，所有读/写操作返回 `kIOReturnNotPermitted` (0xE00002C2)。风扇控制功能在当前 macOS 版本上无法实现。
>
> 详见 [ROADMAP §7 ADR D7](docs/ROADMAP.md#d7--风扇控制移除)。

- ~~**平台**：仅支持 Apple Silicon Mac；检测到 Intel / 非 arm64 时禁用该功能。~~
- ~~**采集**：通过 AppleSMC / IOKit 读取温度和风扇 RPM。~~
- ~~**控制**：固定转速写入范围 `1200...6500 RPM`。~~
- ~~**展示**：状态栏 `49°C` / `1400R`。~~
...
Sampler (单一定时器, .utility QoS, 1~2s)
    │ 后台采集
    ▼
SystemMonitor (actor) ──▶ CPU/Mem/Net/Fan Provider ──▶ Sample (Sendable)
    │ 切回主线程
    ▼
...
```
┌─────────────────────────────────────────────┐
│ …  ↓5.2 ↑0.3  ·  12.4G  ·  38% · 49°C 🔋 ⌘ │
└─────────────────────────────────────────────┘
       └────────── 我们的监控项 ──────────┘
```
- 默认：`↓5.2 ↑0.3 · 12.4G · 38% · 49°C/1400R`
